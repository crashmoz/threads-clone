<template>
	<div id="CreatePost" class="fixed z-50 bottom-0 h-full w-full overflow-hidden	">
		<div class="bg-black h-full text-white overflow-auto">
			<div class="flex items-center justify-start py-4 max-w-[500px] mx-auto border-b border-gray-700">
				<button @click="userStore.isMenuOverlay = false; clearData();" class="rounded-full px-2">
					<Icon name="mdi:close" size="25" />
				</button>
				<div class="text-[16px] font-semobild">New Thread</div>
			</div>
				...
				<div class="py-2 w-full">
					<div class="flex items-center">
						<div v-if="user" class="flex items-center text-white">
							...
							<div class="ml-2 font-semobild text-[18px]">{{ user.identities[0].identity_data.full_name }}</div>
						</div>
					</div>
