<template>
	<div class="z-50 bottom-0 h-full w-full">
		<div class="py-2 w-full">
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center text-white">
					...
					<div class="ml-2 font-semobild text-[18px]">{{ post.name }}</div>
				</div>
				...
				<div v-if="user && user.identities && user.identities[0].user_id == post.userId" @click="isMenu = !isMenu"
					...
					<button :disabled="isDeleting"
						...
						<Icon v-if="!isDeleting" name="bi:three-dots" size="18" color="#ffffff" />
						<Icon v-else name="eos-icons:bubble-loading" size="18" color="#ffffff" />
					</button>
			...
			<div class="relative flex items-center w-full">
				...
				<div class="bg-black rounded-lg w-[calc(100%-50px)] text-sm w-full font-light">
					...
					<div class="absolute mt-2 w-full ml-2">
						<button :disabled="isLike" @click="likeFunc()" class="flex items-center gap-1">
							<Icon v-if="!hasLikeComputed" class="p-1 text-white hover:bg-gray-800 rounded-full cursor-pointer"
		...
		<div class="h-[1px] bg-gray-800 w-full mt-2" />
	</div>
</template>
